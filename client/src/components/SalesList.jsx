...
const SalesList = () => {
  ...
  const fetchSales = async () => {
    const { data } = await axios.get('https://aveshhomemangement.onrender.com/api/sales');
    setSales(data);
  };
  ...
  const handleSave = async () => {
    try { await axios.put(`https://aveshhomemangement.onrender.com/api/sales/${currentSale._id}`, currentSale); handleCloseEdit(); fetchSales(); }
    catch { alert('Failed to update sale'); }
  };
  const handleDelete = async sale => {
    if (!window.confirm('Delete this sale? This will restock all items.')) return;
    try { await axios.delete(`https://aveshhomemangement.onrender.com/api/sales/${sale._id}`); fetchSales(); }
